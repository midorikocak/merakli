    ...
    if ($app->getUsers() == false) {
        ...
        if ($noCategories || $noPosts) {
            if (isset($_SESSION['id']) || (isset($_POST['username']) && isset($_POST['password']))) {
                if ((isset($_SESSION['id']) || $app->login($_POST) != false)) {
                    if ($noCategories && mb_strtolower($_SERVER['REQUEST_URI'])!='/'.$directoryName.'/categories/add' && mb_strtolower($_SERVER['REQUEST_URI'])!='/'.$directoryName.'/users/logout'){
